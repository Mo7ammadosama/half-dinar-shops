...
function cleanup() {
  ...
  // cloudflared can outlive its Node parent on Windows — make sure it's gone.
  try { execSync("taskkill /IM cloudflared.exe /F", { stdio: "ignore" }); } catch {}
  if (backendProc?.pid) {
...
async function main() {
  ...
  for (let attempt = 1; attempt <= MAX_TRIES; attempt++) {
    ...
    // Clear any stale Metro so `expo start` never hits the "port 8081 in use"
    // prompt (which, with no interactive terminal, aborts the dev server).
    const cleared = freePort(8081);
    if (cleared) console.log(`(freed a stale process on port 8081)`);
    ...
    try { execSync("taskkill /IM ngrok.exe /F", { stdio: "ignore" }); } catch {}
